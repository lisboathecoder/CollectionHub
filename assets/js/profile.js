...
async function loadProfile() {
  ...
  try {
    ...
    document.getElementById("userAvatar").src =
      viewingUser.avatarUrl || "/assets/images/icon.png";
    ...
    await Promise.all([loadUserAlbums(userId)]);
  ...
  if (response.ok) {
    ...
    if (albums.length === 0) {
      ...
      albumsGrid.innerHTML = albums
        .map(
          (album) => `
                    <div class="album-card" onclick="window.location.href='/pages/app/album-details.html?id=${
                      album.id
                    }'">
